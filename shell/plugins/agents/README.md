...
|---|---|---|
| `antigravity` | Google Antigravity CLI quota buckets via `agy -p "/usage" --output-format json` (5-hour session + 7-day weekly per model group) | `~/.gemini/antigravity-cli/history.jsonl` prompts and `conversation_summaries.db` sessions |
...
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, and Antigravity via `ANTIGRAVITY_DATA_DIR`. Fireworks reads
